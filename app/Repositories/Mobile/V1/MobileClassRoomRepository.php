...
class MobileClassRoomRepository extends ClassRoomRepository implements IMobileClassRoom
{
    ...
    public function mobileAllSchoolClassroom($params = null): object
    {
        try {
            $this->builder = $this->builder->select(
                'id',
    ...
    public function mobileAllMosqueClassroom($params = null): object
    {
        try {
            $this->builder = $this->builder->select(
                'id',
...
//                ->with('books')
    ...
    public function mobileDeleteObject(string $id): object
    {
        try {
            ...
            // Retrieve the classroom object by ID, throw an exception if not found
            $obj = $this->builder->whereId($id)->firstOrFail();

            // Delete related session attendance records through sessions
            foreach ($obj->sessions as $session) {
                $session->sessionAttendances()->delete();
            }

            // Delete related activity participant records through activities
            foreach ($obj->activities as $activity) {
                $activity->activityParticipants()->delete();
            }

            // Delete all direct relations
            $obj->sessions()->delete();
            ...
            // Finally, delete the classroom object itself
            $obj->delete();

            // Commit the transaction if all deletions succeed
            DB::commit();

            // Return a successful response with the deleted object
            return $this->setResponse(
                parent::SUCCESS,
                $obj,
            );
    ...
    public function mobileGetAllTeachersWithoutClassroom()
    {
        try {
...
//                        ->where('score', '>', 25)
            ...
            if ($propertyType === 'school') {
                $data = [
                    ...
                    //                    [
                    //                        'name' => 'Sessions',
                    //                        'count' => $approvedStudents->sessions_count ?? 0
                    //                    ],
                    //                    [
                    //                        'name' => 'Quizzes',
    ...
    public function mobileCreatClassRoomBook($bookId, $classRoomId): object
    {
        try {
            DB::beginTransaction();
    ...
    public function mobileDeleteClassRoomBook($bookId, $classRoomId): object
    {
        try {
            DB::beginTransaction();
            $classRoom = ClassRoom::findOrFail($classRoomId);
