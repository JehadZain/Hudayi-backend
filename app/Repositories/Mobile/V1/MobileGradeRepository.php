...
class MobileGradeRepository extends GradeRepository implements IMobileGrade
{
        ...
        try {
            $this->builder = $this->builder->select(
                ...
                'classRooms' => function ($query) {
                    $query->select('id', 'name', 'capacity', 'grade_id', 'image', 'is_approved','description')
    ...
    public function mobileById(string $id): object
    {
        try {
            $data = $this->builder->whereId($id)->select(
                ...
                'description',
                'property_id'
            )->with('classRooms:id,name,capacity,grade_id,image,is_approved,description')
