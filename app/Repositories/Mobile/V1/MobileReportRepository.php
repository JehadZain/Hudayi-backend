...
class MobileReportRepository extends ReportRepository implements IMobileReport
{
    public function mobileAll($params = null): object
    {
        try {
            $this->builder = $this->builder->select(
                ...
                'admin_id',
                'report_type_id',
            )->with('reportType:id,name,description')
                ->with('reportReviewers')
                ->with('reportContent');
    ...
    public function mobileCreateObject($object): object
    {
        try {
            ...
            $obj = new $this->model;
            $obj->reportable_type = $object['reportable_type'];
            $obj->reportable_id = $object['reportable_id'];
            $obj->admin_id = $object['admin_id'];
            $obj->report_type_id = $object['report_type_id'];
            $obj->save();
            DB::commit();
