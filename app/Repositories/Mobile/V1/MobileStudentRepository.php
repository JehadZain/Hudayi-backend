...
class MobileStudentRepository extends StudentRepository implements IMobileStudent
{
    ...
    public function mobileById(string $id): object
    {
        try {
            ...
            // Retrieve unattended sessions after enrollment date
            ...
            $unattendedActivities = Activity::whereNotIn('id', $attendedActivityIds)
                ...
                ->get();
            //
            //            dd(Activity::whereIn('id', $attendedActivityIds)
            ...
            // Order Quran quizzes by date in descending order
            //            $quranQuizzes = $data['quran_quizzes']->sortByDesc('id')->values();
            //            $data['quran_quizzes'] = $quranQuizzes;
    ...
    public function mobileGetStudentGroups($params): object
    {
        try {
...
//                }
//                if (!empty($userData['identity_number'])) {
//                    $user->username = $userData['identity_number'];
//                }
//
//                // Check if the image is present in the request
...
//
//                }
//                // Fill user model with other properties from $object['user']
//                $user->fill($userData);
//            }
//            dd($user->image);
//            $user->save();
...
//                    dd($existingRecord);
    ...
    public function studentStatistics(string $id): object
    {
        try {
            ...
            // Get all sessions belonging to the class room within the previous month
//            $classRoomSessions = Session::where('class_room_id', $classRoomId)
////                ->whereBetween('created_at', [$startDate, $endDate])
//                ->get();
//            $unattended_session_count = max($classRoomSessions?->count() - $student?->session_attendances_count, 0);
//
//            // Get all activities belonging to the class room within the previous month
//            $classRoomActivities = Activity::where('class_room_id', $classRoomId)
////                ->whereBetween('created_at', [$startDate, $endDate])
//                ->get();
//            $unattended_activity_count = max($classRoomActivities?->count() - $student?->activity_participants_count, 0);
...
//                ->whereBetween('created_at', [$startDate, $endDate])
    ...
    public function mobileGetAllOrgStudents($params = null): object
    {
        try {
...
//            $filter = $this->$studentsQuery($params);
